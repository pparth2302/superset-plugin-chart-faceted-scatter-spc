...
export function sortFacetValues(values, sortOrder, customOrder) {
  if (sortOrder === void 0) {
    sortOrder = 'asc';
  }
...
export function chunkFacetValuesIntoBalancedRows(values, maxCols) {
  if (!values.length) {
    return [];
  }
...
export function getBalancedFacetLayout(count, maxCols) {
  if (count <= 0) {
    return {
      rows: 0,
      cols: 0,
