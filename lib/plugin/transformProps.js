...
function transformProps(chartProps) {
  var _queriesData$, _getColumnLabel, _ref2, _metricLabels$, _getColumnLabel2, _parseOptionalNumber, _rawFormData$facet_so, _rawFormData$chart_ti, _rawFormData$color_sc, _rawFormData$show_leg, _rawFormData$time_for;
  var {
    width,
    height,
    queriesData,
  ...
  var yAxisLabel = (_ref2 = (_metricLabels$ = metricLabels[0]) != null ? _metricLabels$ : (0, _utils.getColumnLabel)(rawFormData.y_axis_column)) != null ? _ref2 : 'value';
  ...
  var panels = sortedFacetValues.map(facetValue => {
    var points = data.reduce((accumulator, row) => {
      ...
      var yValue = Number(row[yAxisLabel]);
      ...
      accumulator.push({
        ...
        tooltipValues: [{
          label: xAxisLabel,
          value: row[xAxisLabel]
        }, {
          label: facetColumnLabel,
          value: facetValue
        }, {
          label: yAxisLabel,
          value: row[yAxisLabel]
        }, ...(colorColumnLabel && row[colorColumnLabel] != null ? [{
          label: colorColumnLabel,
          value: row[colorColumnLabel]
        }] : []), ...tooltipColumnLabels.filter(label => ![xAxisLabel, facetColumnLabel, yAxisLabel, colorColumnLabel].includes(label)).map(label => ({
          label,
          value: row[label]
        }))],
        row
